...
Zmienna poj_h zostanie usunięta ze względu na zerową wariancję.
...
Dane dzielone są na zbiór uczący (70%) i testowy (30%). Dodatkowo tworzona jest informacja o godzinie i dniu tygodnia.
...
Ze względu na bardzo wysoką skośność prawostronną konieczne jest zastosowanie transformacji Yeo-Johnsona
...
Z par zmiennych objaśniających o wspólczynniku korelacji powyżej 0.9 zostanie usunięta zmienna, która mniej koreluje ze zmienną objąsnianą. 
...
### Najważniejsze predyktory vs zmienna objaśniana
...
W modelach regresji liniowych należy uwzględnić nieliniowość relacji.
...
Dla wyższych wartości temperatur relacja między prędkością wiatru a grimm_pm10 zmienia się.

### Interakcje między pogodą
...
W ostatecznym przepisie uwzględniono:

- Transformacje trygonometryczne czasu,

- Interakcje między zmiennymi pogodowymi i cząsteczkami,

- Normalizację i transformację Yeo–Johnsona,
...
## Przepis z PCA (model ma mniej zmiennych do analizy, zachowuje najwazniejsze "n_"
...
Testowe przygotowanie danych dla każdego przepisu XGB i podgląd ich struktury.
...
Ranking wyników strojenia i wybór najlepszej kombinacji.
...
Definicja siatki hiperparametrów i metryk dla SVR.
...
Uruchomienie procesu strojenia dla wszystkich wariantów SVR.
...
rec <- recipe(grimm_pm10 ~ ., data = train_tr) %>%
  step_zv(all_predictors()) %>%             # usuń predyktory bez zmienności
  step_dummy(all_nominal_predictors()) %>%  # zamień zmienne kategoryczne na zmienne 0/1
...
## Walidacja z użyciem rolling-origin CV
...
## Obliczenie współczynników i VIF

```{r}
lm_obj <- lm_final %>% pull_workflow_fit() %>% .$fit
vif_safe <- function(model) { # obejście błędu przy car::vif()
...
## Współczynniki regresji (posortowane wg wartości p)
...
Tabelaryczne i graficzne porównanie metryk (RMSE, R-squared, MAE) dla wszystkich trzech modeli na zbiorze testowym.
...
## Uzupełnienie ewentualnych brakujących kolumn (np. hour, wday)
...
## Wypełnienie NA medianami (jak w treningu)
